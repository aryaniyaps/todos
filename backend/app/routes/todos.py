...
def get_todo(
    current_user: User, todo_id: int, session: Session = Depends(get_session)
) -> Todo:
    todo = TodoService(session).get_todo(todo_id=todo_id, user_id=current_user.id)
    if todo is None:
        raise HTTPException(
...
def read_todos(current_user: User, session: Session = Depends(get_session)):
    """
...
def create_todo(
    current_user: User, data: TodoCreate, session: Session = Depends(get_session)
):
    """
    Create a new todo.
    """
    return TodoService(session).create_todo(
        content=data.content, completed=data.completed, user_id=current_user.id
    )
...
@todo_router.delete(
    "/{todo_id}", name="todos:delete", status_code=HTTPStatus.NO_CONTENT
)
def delete_todo(
    todo: Todo = Depends(get_todo), session: Session = Depends(get_session)
):
    """
    Delete a todo by ID.
    """
...
def clear_todos(current_user: User, session: Session = Depends(get_session)):
    """
